...
from utils import frame_to_world, get_closest_block
...
class Gui(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        #task buttons
        self.ui.btnUser4.setText('Save Open Gripper')
        self.ui.btnUser4.clicked.connect(partial(nxt_if_arm_init, 'save_open_gripper'))

        self.ui.btnUser5.setText('Save Close Gripper')
        self.ui.btnUser5.clicked.connect(partial(nxt_if_arm_init, 'save_closed_gripper'))

        self.ui.btnUser6.setText('Click and Grab')
        self.ui.btnUser6.clicked.connect(partial(nxt_if_arm_init, 'click_and_grab_idle'))
        
        self.ui.btnUser7.setText('Competition 1')
        self.ui.btnUser7.clicked.connect(partial(nxt_if_arm_init, 'competition1'))

        self.ui.btnUser8.setText('Competition 2')
        self.ui.btnUser8.clicked.connect(partial(nxt_if_arm_init, 'competition2'))

        self.ui.btnUser9.setText('Competition 3')
        self.ui.btnUser9.clicked.connect(partial(nxt_if_arm_init, 'competition3'))

        self.ui.btnUser10.setText('Competition 3 test')
        self.ui.btnUser10.clicked.connect(partial(nxt_if_arm_init, 'competition3_test'))

        self.ui.btnUser12.setText('Competition 4')
        self.ui.btnUser12.clicked.connect(partial(nxt_if_arm_init, 'competition4'))


        self.ui.btnUser11.setText('Snap Picture')
        self.ui.btnUser11.clicked.connect(partial(nxt_if_arm_init, 'snap_picture'))

        self.ui.btn_task1.setText('Detect')
        self.ui.btn_task1.clicked.connect(partial(nxt_if_arm_init, 'detect'))

        self.ui.btn_task2.setText('Save Waypoints')
        self.ui.btn_task2.clicked.connect(partial(nxt_if_arm_init, 'save_waypoints'))

       
        self.ui.btn_task3.setText('Start Recording')
        self.ui.btn_task3.clicked.connect(partial(nxt_if_arm_init, 'teach_and_repeat_initialize_teach'))

        self.ui.btn_task4.setText('Stop Recording')
        self.ui.btn_task4.clicked.connect(partial(nxt_if_arm_init, 'teach_and_repeat_end_teach'))

        self.ui.btn_task5.setText('Repeat')
        self.ui.btn_task5.clicked.connect(partial(nxt_if_arm_init, 'teach_and_repeat_initialize_repeat'))
    ...
    def trackMouse(self, mouse_event):
        ...
        pt = mouse_event.pos()
        pixel = np.array([pt.x(), pt.y(), 1], dtype=np.float32)
        ...
        if self.camera.DepthFrameRaw.any() != 0:
            ...
            if self.camera.camera_calibrated:
                ...
                world_to_camera_transition = [60, 393, 990]
                extrinsic_matrix = np.array([
                    [rotation_matrix[0][0], rotation_matrix[0][1], rotation_matrix[0][2], world_to_camera_transition[0]],
                    [rotation_matrix[1][0], rotation_matrix[1][1], rotation_matrix[1][2], world_to_camera_transition[1]],
                    [rotation_matrix[2][0], rotation_matrix[2][1], rotation_matrix[2][2], world_to_camera_transition[2]],
                    [0, 0, 0, 1]
                ], dtype=np.float32)
                extrinsic_matrix = np.linalg.inv(extrinsic_matrix)
                
            world_vec = frame_to_world(pixel, 
            ...
            self.ui.rdoutMouseWorld.setText("(%.0f,%.0f,%.0f)" %
                                             (world_vec[0], world_vec[1], world_vec[2]))

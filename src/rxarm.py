...
from constant import STRAIGHT_UP
...
class RXArm(InterbotixManipulatorXS):
    ...
    def set_positions(self, joint_positions, is_comp4=False):
        """!
         @brief      Sets the positions.

         @param      joint_angles  The joint angles
         """
        if (joint_positions == STRAIGHT_UP):
            self.set_accel_time(0.5)
            self.set_moving_time(1.2)
            self.set_straight_up()
            return
        sum_angle_max = -1 # Larger sum_angle_max == larger movement
        for i, joint_position in enumerate(joint_positions):
            sum_angle_max = max(abs(joint_position - self.position_fb[i]), sum_angle_max)
        sum_angle_ratio = sum_angle_max / 1.3 # TODO: test this value
        # print("SUM ANGLE DIFF: ", sum_angle_max)
        # print("SUM ANGLE RATIO: ", sum_angle_ratio)
        if is_comp4:
            sum_angle_ratio_cap = 1.2 # TODO: may not need upper bound with max
            if (sum_angle_ratio > sum_angle_ratio_cap): sum_angle_ratio = sum_angle_ratio_cap
            elif (sum_angle_ratio < 0.45): sum_angle_ratio = 0.45 # set a lower bound to be 1
            new_accel_time = 0.5 * sum_angle_ratio # Default is 0.5
            new_moving_time = 1.5  * sum_angle_ratio # Default is 2
            self.set_accel_time(new_accel_time)
            self.set_moving_time(new_moving_time)
            # print(f"{new_accel_time=} {new_moving_time=}")
            ...
            sum_angle_ratio_cap = 1.4 # TODO: may not need upper bound with max
            if (sum_angle_ratio > sum_angle_ratio_cap): sum_angle_ratio = sum_angle_ratio_cap
            elif (sum_angle_ratio < 0.5): sum_angle_ratio = 0.5 # set a lower bound to be 1
            new_accel_time = 0.5 * sum_angle_ratio # Default is 0.5
            new_moving_time = 2  * sum_angle_ratio # Default is 2
            self.set_accel_time(new_accel_time)
            self.set_moving_time(new_moving_time)
            # print(f"{new_accel_time=} {new_moving_time=}")
            self.arm.set_joint_positions(joint_positions,
    ...
    def set_straight_up(self):
        self.arm.set_joint_positions(STRAIGHT_UP, moving_time = 1.2, accel_time = 0.5, blocking=False)
    ...
    def get_total_move_time(self):
        """!
        @brief      Gets the total move time (moving time + accel time).

        @return     The total move time.
        """
        return self.moving_time + self.accel_time
